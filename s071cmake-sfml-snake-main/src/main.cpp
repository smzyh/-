...
using namespace sf;			//SFML 中的每个类都位于该命名空间之下
bool isGameOver, isGameQuit;
bool updateFlag;
constexpr int mWidth = STAGE_WIDTH;
constexpr int mHeight = STAGE_HEIGHT;
...
//这里我们还没有用到类的封装，所以暂时把 window 作为全局变量。
//对新建窗口的封装，第一个参数是一个 VideoMode，表示窗口大小，第二个表示窗口标题。实际上最多可以使用四个参数，最后两个是可选的 -  Style 和 ContextSettings。
//标题中有宽字符的，字符串前要加 L，不然会显示乱码
RenderWindow window{VideoMode{{mWidth * GRIDSIZE + INFO_WIDTH, mHeight * GRIDSIZE + GRIDSIZE}}, L"Snake by 李仕"};
...
Texture tBackground, tSnakeHead, tSnakeBody, tFruit;		//创建 4 个纹理对象
Sprite spBackground, spSnakeHead, spSnakeBody, spFruit;		//创建 4 个精灵对象
...
void Initial()
{
	...
	spBackground.setOrigin(sf::Vector2f{GRIDSIZE / SCALE / 2, GRIDSIZE / SCALE / 2});
	spSnakeHead.setOrigin(sf::Vector2f{GRIDSIZE / SCALE / 2, GRIDSIZE / SCALE / 2});
	spSnakeBody.setOrigin(sf::Vector2f{GRIDSIZE / SCALE / 2, GRIDSIZE / SCALE / 2});
	spFruit.setOrigin(sf::Vector2f{GRIDSIZE / SCALE / 2, GRIDSIZE / SCALE / 2});

	spBackground.setScale(sf::Vector2f{SCALE, SCALE});
	spSnakeHead.setScale(sf::Vector2f{SCALE, SCALE});
	spSnakeBody.setScale(sf::Vector2f{SCALE, SCALE});
	spFruit.setScale(sf::Vector2f{SCALE, SCALE});
...
void Input()
{
	...
	while (const std::optional event = window.pollEvent())
	{
		...
		if (const auto* KeyReleased = event->getIf<sf::Event::KeyReleased>())
		{
			...
			{
				window.close();		//按 Escape 调用 close() 函数退出程序
				isGameOver = false;
				isGameQuit = true;
			}
			...
			if (KeyReleased->scancode == sf::Keyboard::Scancode::Slash || KeyReleased->scancode == sf::Keyboard::Scancode::Enter)
			{
				if (MusicOn == true)
				{
